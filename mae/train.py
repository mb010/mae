...
from architectures.vit import Transformer, ViT
...
def run_pretraining(config, datamodule, experiment_dir, wandb_logger):
    ...
    encoder = ViT(
        ...
        dim=config["architecture"]["encoder"]["dim"],
        depth=config["architecture"]["encoder"]["depth"],
        heads=config["architecture"]["encoder"]["heads"],
        mlp_dim=config["architecture"]["encoder"]["mlp_dim"],
    )

    decoder = Transformer(
        dim=config["architecture"]["decoder"]["dim"],
        depth=config["architecture"]["decoder"]["depth"],
        heads=config["architecture"]["decoder"]["heads"],
        dim_head=config["architecture"]["decoder"]["dim_head"],
    )
