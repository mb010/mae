...
def _decoder(preset, embed_dim, depth, num_heads, mlp_ratio):
    if preset.lower() == "none":
        return Transformer(embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio)
    elif preset == "default":
        return Transformer(embed_dim=512, depth=8, num_heads=16, mlp_ratio=4)
    elif preset == "small":
        return Transformer(embed_dim=256, depth=4, num_heads=8, mlp_ratio=4)


def vit_small(patch_size, img_size, in_chans, **kwargs):
    model = ViT_Encoder(
...
def vit_base(patch_size, img_size, in_chans, **kwargs):
    model = ViT_Encoder(
        patch_size=patch_size,
        img_size=img_size,
        in_chans=in_chans,
...
def vit_large(patch_size, img_size, in_chans, **kwargs):
    model = ViT_Encoder(
        patch_size=patch_size,
        img_size=img_size,
        in_chans=in_chans,
...
def _encoder(preset, patch_size, img_size, in_chans, **kwargs):
    if preset == "small":
        return vit_small(patch_size, img_size, in_chans, **kwargs)

    elif preset == "base":
        return vit_base(patch_size, img_size, in_chans, **kwargs)

    elif preset == "large":
        return vit_large(patch_size, img_size, in_chans, **kwargs)

    elif preset.lower() == "none":
        return ViT_Encoder(patch_size, img_size, in_chans, **kwargs)
